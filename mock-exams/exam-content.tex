...
\begin{tikzpicture}[>=Latex,node distance=1.5cm,
   block/.style={draw,rectangle,minimum height=0.95cm,minimum width=1.25cm},
   sum/.style={draw,circle,inner sep=0pt,minimum size=6.5mm}]
 \node (in) {$R(s)$};
 \node[sum,right=0.9cm of in] (s1) {};
 \node[block,right=1.1cm of s1] (g1) {$\dfrac{2}{s+1}$};
 \node[sum,right=1.1cm of g1] (s2) {};
 \node[block,right=1.1cm of s2] (g2) {$\dfrac{5}{s+4}$};
 \coordinate (p) at ($(g2.east)+(0.55,0)$);
 \coordinate (q) at ($(p)+(0.6,0)$);
 \node[right=1.7cm of g2] (out) {$Y(s)$};
 \node[block,below=1.0cm of g2] (h2) {$2$};
 \draw[->] (in) -- (s1);
 \draw[->] (s1) -- node[above,pos=0.15]{\small$+$} (g1);
 \draw[->] (g1) -- node[above,pos=0.15]{\small$+$} (s2);
 \draw[->] (s2) -- (g2);
 \draw (g2) -- (q);
 \draw[->] (q) -- (out);
 \fill (p) circle (1.4pt);
 \fill (q) circle (1.4pt);
 % inner feedback H2 = 2 (negative into s2 from below)
 \draw[->] (p) |- (h2.east);
 \draw[->] (h2.west) -| node[pos=0.95,left]{\small$-$} (s2.south);
 % outer unity feedback (negative into s1 from below)
 \draw[->] (q) -- ++(0,-2.05) -| node[pos=0.95,left]{\small$-$} (s1.south);
\end{tikzpicture}\\[3pt]
